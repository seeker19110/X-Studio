"""Sandbox tiến trình cho lệnh con của phòng video (ADR-0035 của software-company).

**BẢN TẠM — SẼ XOÁ Ở K3.2.** File này là bản sao gần như nguyên văn của `software-company/src/company/sandbox.py`
(cùng interface: `RunSpec`, `Result`, `Handle`, `Sandbox`, `SubprocessSandbox`, `ContainerSandbox`,
`sandbox_from_config`, `sanitize_env`). Nó tồn tại vì hai package chưa có lõi chung: K3.2 đưa module này vào
`xagents-core` và cả hai bên chỉ còn `from xagents_core.sandbox import ...`. Đừng cho nó rẽ nhánh khỏi bản của
company — sửa gì thì sửa cả hai, hoặc chờ K3.2.

Khác bản company đúng bốn chỗ, đều vì lớp media:

1. `clean_env`/`SECRET_ENV` nằm ngay đây (studio không có `workspace.py`).
2. `RunSpec.stdin`: `CommandTTS` đưa văn bản vào stdin của lệnh TTS. Với `ContainerSandbox`, stdin đã bị
   `--env-file -` chiếm, nên khi có `stdin` thì env đi bằng `-e KEY=VALUE` (giá trị hiện trong danh sách tiến
   trình của máy — đánh đổi được ghi ở đây để không ai tưởng là kín).
3. `RunSpec.read_only`: qc chỉ đo file, mount `:ro`.
4. Mặc định của `sandbox_from_config` là `subprocess`, KHÔNG phải `auto`: ffmpeg/ffprobe nhận đường dẫn tuyệt đối
   trải trên nhiều thư mục (output, font, thư mục tạm), mà container chỉ thấy `cwd` được mount. Muốn container thì
   phải khai đích danh `render.sandbox.mode: container` và tự lo mount — không tự bật sau lưng người dùng.
"""
...
def sandbox_from_config(cfg: Any = None, which: Any = shutil.which) -> Sandbox:
    ...
    if mode == "container":
        if not which(runtime):
            raise SandboxError(f"STUDIO_SANDBOX=container nhưng không tìm thấy `{runtime}` trên PATH; "
                               f"cài runtime hoặc đặt STUDIO_SANDBOX=subprocess (không tự tụt hạng bảo vệ)")
