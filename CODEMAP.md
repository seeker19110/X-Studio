...
| Ghi/phát lại eval, cổng bản ghi | `xagents-core/src/xagents_core/evals.py` từ K3.6c — `src/studio/evals.py` giữ `main` (cổng: đỏ khi ca không chạy được, `CaseResult.errored`) và `_run_case` (`many`/`extra`) | `xagents-core/tests/test_evals.py`, `tests/test_evals_cli.py` |
| Hình dạng JSON model phải trả | `output_schema` + `context_writes_schema` ở `src/studio/runner.py` (studio KHÔNG đòi `content`) — sửa là mọi bản ghi eval lệch; khung ở `xagents-core/src/xagents_core/runner.py` từ K3.6d1 | `xagents-core/tests/test_runner.py` |
| Vòng đời một lượt agent | `xagents-core/src/xagents_core/runner.py` `AgentRunner` từ K3.6d2 — `src/studio/runner.py` giữ `generate` + vòng lặp tool và hai hook (`_audit_scope`, `_produced_evidence`); **KHÔNG** bật `wants_content`, **KHÔNG** dùng `inp.child()` — đổi hai thứ đó là đổi hành vi studio, có ca chốt | `xagents-core/tests/test_runner.py`, `tests/test_runner_core.py` |
