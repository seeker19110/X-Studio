...
| ~~`claude -p` không tool `--max-turns 1`~~ **đã vá 2026-09-09** | `make eval-record` bằng model thật chết `error_max_turns` ở `seo-optimizer` (1/2 ca). Company đã đo và vá đúng lỗi này 2026-09-05 nhưng studio không được port — bản ghi eval studio vì thế chưa từng chạy lại bằng model thật | `--json-schema` ép JSON bằng một lượt CLI nữa; `CLI_NO_TOOL_TURNS = 6`. Lần sau vá lỗi provider ở một công ty, **grep công ty kia ngay** — hai adapter `claude-code` vẫn là hai bản |
